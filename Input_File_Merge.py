...
def merge_input_files(file_0101, file_0151, input_file, output_file):
    ...
    for var in ds_merged.data_vars:
        ...
        size_gb = ds_merged[var].nbytes / (1024**3)
        print(f"{var}: {dtype}, {size_gb:.2f} GB")

    encoding = {}
    for var in ds_merged.data_vars:
        if var == 'PRECT':
            # Convert PRECT from float64 to float32 (saves ~29GB)
            target_dtype = 'float32'
        else:
            # TS and Z500 are already float32
            target_dtype = ds_merged[var].dtype
        
        encoding[var] = {
            'zlib': True,
            'complevel': 5,  # Good compression without being too slow
            'shuffle': True,
            'fletcher32': False,
            'dtype': target_dtype
        }
...
merge_input_files(
    ...
    input_file = '',
    output_file='/pscratch/sd/p/plutzner/E3SM/bigdata/input_vars.P_T_Z5.v2.LR.historical_merged_0101_0151.eam.h1.1685-2014.nc'
)
