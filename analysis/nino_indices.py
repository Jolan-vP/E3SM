...
def rolling_ave(da, averaginglength):
    if len(da.shape) == 1: 
        return da.rolling(time = averaginglength, center = False).mean() # Whether or not to center the rolling mean??? 
    else: 
        da_copy = da.copy()
        inc = 45
...
def NinoIndices(dailyE3SMdata, averaginglength, keyword = None):
    """
    Inputs: E3SM Daily Test Data
    ...
    # Open test data to calculate Nino Indices (Analysis is performed on verification day so test data is the appropriate data)
    # daily_test_data = '/pscratch/sd/p/plutzner/E3SM/bigdata/input_vars.v2.LR.historical_0201.eam.h1.1850-2014.nc'
    da = xr.open_dataset(dailyE3SMdata) 
    da_sst = da["TS"] # sea surface temperature

    # (2) Take Area Weighted Average of TS over the lat lon box regions for EACH Index:
    TS3_4 = _extractregion(da_sst, nino_boxbounds[0,:])
    TS1_2 = _extractregion(da_sst, nino_boxbounds[1,:])
    TS3 = _extractregion(da_sst, nino_boxbounds[2,:])
    TS4 = _extractregion(da_sst, nino_boxbounds[3,:])
    ...
    for key in temp_dict:
        if key == "Nino34":
            ...
            print(f"Temp dict time: {temp_dict[key].time}")
            ...
            # Save .nc file output
            print(f'File will be saved to: /pscratch/sd/p/plutzner/E3SM/bigdata')
            temp_dict[key].to_netcdf('/pscratch/sd/p/plutzner/E3SM/bigdata/Nino34_Indices_E3SM_' + str(keyword) + '.nc')
        else:
            pass
    return temp_dict
...
fn =  '/pscratch/sd/p/plutzner/E3SM/bigdata/input_vars.v2.LR.historical_0201.eam.h1.1850-2014.nc'
averaging_length = 30 * 5 # 5 month running mean of daily data

ninox0201 = NinoIndices(fn, averaginglength = averaging_length, keyword = '0201')

plt.figure(figsize=(12, 6))
plt.plot(ninox0201["Nino34"].time, ninox0201["Nino34"], label = "Nino34")
plt.title("Nino34 Indices")
plt.xlabel("Time")
plt.ylabel("Nino34 Indices")
plt.legend()
plt.show()
plt.savefig('/pscratch/sd/p/plutzner/E3SM/bigdata/Nino34_Indices_E3SM_0201.png', dpi=300, bbox_inches='tight')
plt.close()
