...
if config["data_source"] == "E3SM": 
    ...
    daily_ENSO = xr.open_dataset('/pscratch/sd/p/plutzner/E3SM/bigdata/Nino34_Indices_E3SM_0201.nc')
    Nino34 = daily_ENSO.TS.sel(time=slice ( str(config["databuilder"]["input_years"][0]) + '-01-01', str(config["databuilder"]["input_years"][1]) + '-12-31'))
...
#     Nino34 = Nino34
    ...
    monthlyENSO = xr.open_dataset('/pscratch/sd/p/plutzner/E3SM/bigdata/ERA5/nino34.long.anom.nc')
    Nino34 = monthlyENSO.value

# Calculate enso indices for daily data based on EVALUATION DAY
daily_enso_timestamps = ENSO_indices_calculator.identify_nino_phases(Nino34, config, threshold=0.4, window=6, lagtime = lagtime, smoothing_length = smoothing_length)
elnino_dates = daily_enso_timestamps["El Nino"]
lanina_dates = daily_enso_timestamps["La Nina"]
neutral_dates = daily_enso_timestamps["Neutral"]
...
# analysis_metrics.IQR_success_discard_plot(output, CRPS_network, CRPS_climatology, config, keyword = None)
...
high_conf_EN_dates = elnino_dates[elnino_dates.isin(IQR_subset_highconf_dates)]
# EL NINO - Low Confidence
low_conf_EN_dates = elnino_dates[elnino_dates.isin(IQR_subset_lowconf_dates)]
...
high_conf_LN_dates = lanina_dates[lanina_dates.isin(IQR_subset_highconf_dates)]
# LA NINA - Low Confidence
low_conf_LN_dates = lanina_dates[lanina_dates.isin(IQR_subset_lowconf_dates)]
...
high_conf_NE_dates = neutral_dates[neutral_dates.isin(IQR_subset_highconf_dates)]
# Neutral - Low Confidence
low_conf_NE_dates = neutral_dates[neutral_dates.isin(IQR_subset_lowconf_dates)]
...
# analysis_metrics.differenceplot(Z500_complowCRPS_highconf_dates, Z500_complowCRPS_lowconf_dates, Z500_test_data, CRPS_network, config, normalized = True, keyword= "Comparatively Low CRPS High-Low Confidence Z500 Norm")
# analysis_metrics.differenceplot(Z500_complowCRPS_highconf_dates, Z500_complowCRPS_lowconf_dates, Z500_test_data, CRPS_network, config, normalized = False, keyword= "Comparatively Low CRPS High-Low Confidence Z500")
...
# sub_EN_dates, sub_LN_dates, sub_NE_dates = analysis_metrics.subsetanalysis_SHASH_ENSO(CNN_crps_da_lower_index, output, climatology, target, target_raw, config, x, subset_keyword = 'CNN < SNN')
...
# sublowest_EN_dates, sublowest_LN_dates, sublowest_NE_dates = analysis_metrics.subsetanalysis_SHASH_ENSO(CNN_crps_da_lowest_index, output, climatology, target, target_raw, config, x, subset_keyword = 'Lowest CNN CRPS < SNN CRPS')
