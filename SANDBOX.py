...
if config["network_type"] == "simple":
    crps_SNN = CRPS_network
    CNN_expname = "" # CHOOSE
    crps_CNN = open_data_file(str(config["perlmutter_output_dir"]) + str(CNN_expname) + '/' + str(CNN_expname) + '_network_SHASH_parameters.pkl')
else:
    crps_CNN = CRPS_network
    SNN_expname = "" # CHOOSE
    crps_SNN = open_data_file(str(config["perlmutter_output_dir"]) + str(SNN_expname) + '/' + str(SNN_expname) + '_network_SHASH_parameters.pkl')

# Discard plot of CRPS vs Target Magnitude; CNN, Simple NN, Climo
analysis_metrics.target_discardplot(target, crps_CNN, crps_SNN, CRPS_climatology, config, target_type = 'anomalous')
