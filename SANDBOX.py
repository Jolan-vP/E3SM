...
config = utils.get_config("exp032")
...
data = ClimateData(
    ...
    target_only = False, ## CHECK FOR EACH NETWORK!!
    fetch=False,
    verbose=False
)
...
# # Trim data for faster training FOR NOW: 
# train_dat = train_dat.sel(time = slice(0, 10000))
# val_dat = val_dat.sel(time = slice(0, 10000))
# test_dat = test_dat.sel(time = slice(0, 10000))

# # Confirm data looks correct: 
# print(f"shape of train input data: {train_dat['x'].shape}")
