...
config = utils.get_config("exp020")
...
data = ClimateData(
    ...
    seed=seed,
    data_dir = config["perlmutter_inputs_dir"], 
...
s_dict_savename1 = str(config["perlmutter_inputs_dir"]) + 'exp006_train_unlagged_28.pkl'
...
s_dict_savename2 = str(config["perlmutter_inputs_dir"]) + 'exp006_val_unlagged_28.pkl'
...
s_dict_savename3 = str(config["perlmutter_inputs_dir"]) + 'exp006_test_unlagged_28.pkl'
...
for i, m in enumerate(("loss", *config["metrics"])):
    ...
    plt.legend()
plt.tight_layout()
plt.savefig(str(config["perlmutter_figure_dir"]) + str(config["expname"]) + '/training_metrics.png', format = 'png', dpi = 300)
plt.show()
...
climatology = universaldataloader(s_dict_savename1, config, target_only = True)
