...
config = utils.get_config("exp068")
...
elif config["input_data"] == "uniform":
    # Generate uniform distribution data
    # Find length of standard input data using exp042
    input_trainfn = str(config["perlmutter_inputs_dir"]) + "exp042" + "_trimmed_" + "train_dat.nc"
    cnn_input_train = open_data_file(input_trainfn)
    climo_len = cnn_input_train['y'].shape[0]
    dist = uniform_dist(-7, 7, climo_len, config["expname"], config)
    print(dist)
    print(f"uniform dist shape: {dist.shape}")
